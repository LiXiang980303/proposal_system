    ...
    <script type="text/javascript">
	    ...
	    var prop = {"rows":[
            {"name":"字段名称","id":"zt_columnname","value":"","group":"业务对象属性","editor":{
         	...
         	{"name":"是否单选","id":"zt_singleselect","value":"true","group":"独立属性","editor":{
         		"type":"checkbox",
         		"options":{
         			"on":true,
         			"off":false
         	...
         	{"name":"url","id":"zt_url","value":"","group":"独立属性","editor":{
         		"type":"textbox",
         		"options":{
...
<body style="margin:0;padding:0;" >
	...
	<script type="text/javascript">
		...
		var oNode = null,thePlugins = 'combotree';
		...
		function onok (){
	        ...
	        ed = $('#dg').propertygrid('getEditor',{index:propmap.zt_url,field:'value'});
	        $(oNode).attr('zt_url',$(ed.target).textbox('getValue'));
		...
		function init(){
			...
			if(proploaded){
			    ...
			    if( UE.plugins[thePlugins].editdom ){
		    		...
		    		var gUrl = $(oNode).attr('zt_url');
		    		ed = $('#dg').propertygrid('getEditor',{index:propmap.zt_url,field:'value'});
		    		$(ed.target).textbox('setValue', gUrl);
