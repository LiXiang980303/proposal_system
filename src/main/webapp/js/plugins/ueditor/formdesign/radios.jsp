		...
		function init(){
			...
			if(proploaded){
			    ...
			    if( UE.plugins[thePlugins].editdom ){
		    		...
		    		var radios = $(oNode).find("input");
		    		for(var i=0;i<radios.length;i++){
		    			if(oNode.childNodes[2*i+1].nodeType == 3){
		    				addNewItem(oNode.childNodes[2*i+1].nodeValue,$(radios[i]).val());
		    			}
		    		}
		...
		function activeItem(){
			if(validEdit()){
				...
				for(var i=0;i<rows.length;i++){
					...
					var opt = $("<input name='"+$(ed.target).combobox('getValue')+"' value='"+$(valueed.target).textbox('getValue')+"' type='radio' style='margin-right:2px' />");
					$(oNode).append(opt);
